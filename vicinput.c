...
const char flist[BUFSIZ+1]="./cmd/latlong99.txt";
		    ...
		    float *tempmin, int cl)
{
 ...
 for (i=0;i<cl;i++)
   {
   if (fscanf(fppp,"%f",&precip[i])!=1)  printf("Error prcp1\n"); 
   if (fscanf(fpta,"%f",&tempmax[i])!=1) printf("Error tmax1\n");
   if (fscanf(fpti,"%f",&tempmin[i])!=1) printf("Error tmin1\n");
   if (tempmax[i]<tempmin[i]) {
     ttemp = tempmax[i];
     tempmax[i] = tempmin[i];
     tempmin[i] = ttemp;}
